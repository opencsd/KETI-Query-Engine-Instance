...
void DBConnectorInstance::handle_get(http_request message)
{
    ...
    if (strcmp(INSTANCE_TYPE, "MYSQL") == 0) {
        if(url == "/metadata/schema"){
            
        }else if(url == "/metadata/environment"){
...
{    
    ...
    if(strcmp(INSTANCE_TYPE, "MYSQL") == 0){
        if(url == "/query/run"){
            ...
            string query, db_name, db_user, db_password, uid;

            if(document.HasMember("query")){
                query = document["query"].GetString();
                cout << "[QueryEngine] input mysql query> " << query << endl;
            }
            if(document.HasMember("db_name")){
                db_name = document["db_name"].GetString();
            }
            if(document.HasMember("db_user")){
                db_user = document["db_user"].GetString();
            }
            if(document.HasMember("db_password")){
                db_password = document["db_password"].GetString();
            }
            if(document.HasMember("uid")){
                uid = document["uid"].GetString();
            }
        
            QueryLog query_log(query, db_name, uid);
            query_log.CheckMetricBefore();

            string db_host = "tcp://" + string(STORAGE_ENGINE_IP) + ":3306";

            string result_str = "";

            try {
                sql::mysql::MySQL_Driver *driver = sql::mysql::get_mysql_driver_instance();
                sql::Connection *con = driver->connect(db_host, db_user, db_password);
                ...
                vector<int> colWidths(columnCount, 15);

                result_str = formatTable(res);
                
                delete res;
                delete stmt;
                delete con;
            } catch (sql::SQLException &e) {
                KETILOG::ERRORLOG("MySQL Connection Error::DB Connectior Instance", e.what());
            }
            ...
            MetaDataManager::UpdateEnvInfo(block_count, scheduling_algorithm);
        ...
        if(url == "/metadata/environment"){ // 하드코딩 환경정보 처리 필요
            ...
            string query, db_name, user_name, uid;
            ...
            cout << "[QueryEngine] input query> " << parsed_query.GetParsedQuery() << endl;
            ...
            QueryLog query_log(user_name, parsed_query.GetParsedQuery(), parsed_query.GetExecutionMode(), parsed_query.GetQueryType(), db_name, uid);
